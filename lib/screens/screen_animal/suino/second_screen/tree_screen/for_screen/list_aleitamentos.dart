...
class _ListaAleitamentosState extends State<ListaAleitamentos> {
  ...
  List<Aleitamento> totalAleitamentos = List();
  ...
  void _showTotalBezerrasPage({Aleitamento aleitamento}) async {
    final recAleitamentos = await Navigator.push(
        context,
        MaterialPageRoute(
    ...
    if (recAleitamentos != null) {
      if (totalBezerras != null) {
        await helper.updateItem(recAleitamentos);
      } else {
        await helper.insert(recAleitamentos);
      }
      _getAllBezerras();
    }
  }
