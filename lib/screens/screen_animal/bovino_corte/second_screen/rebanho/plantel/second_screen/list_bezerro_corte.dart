...
import 'package:gerenciamento_rural/screens/screen_animal/bovino_corte/second_screen/rebanho/plantel/second_screen/registers/cadastro_bezerro_corte.dart';
...
class ListaBezerroCorte extends StatefulWidget {
  @override
  _ListaBezerroCorteState createState() => _ListaBezerroCorteState();
}

class _ListaBezerroCorteState extends State<ListaBezerroCorte> {
  TextEditingController editingController = TextEditingController();
  BezerroCorteDB helper = BezerroCorteDB();
  List<BezerroCorte> totalBez = [];
  List<BezerroCorte> items = [];
  List<BezerroCorte> tBezerros = [];
  @override
  void initState() {
    super.initState();
    _getAllBez();
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        centerTitle: true,
        title: Text("Bezerros da Propriedade"),
      ),
      body: Container(
        child: Column(
          children: [
  ...
  void _showtotalBezPage({BezerroCorte totalBez}) async {
    final rectotalBez = await Navigator.push(
        context,
        MaterialPageRoute(
          builder: (context) => CadastroBezerroCorte(
            bezerro: totalBez,
          ),
        ));
    if (rectotalBez != null) {
  ...
  void _getAllBez() {
    ...
    items.forEach((element) {
      ...
      if (quant >= 720) {
        GarroteCorte garroteCorte;
        GarroteCorteDB garroteCorteDB = GarroteCorteDB();
        element.virouAdulto = 1;
        helper.updateItem(element);
        garroteCorte = GarroteCorte.fromMap(element.toMap());
        garroteCorteDB.insert(garroteCorte);
      }
    });
  }
  ...
  void filterSearchResults(String query) {
    List<BezerroCorte> dummySearchList = [];
    dummySearchList.addAll(items);
    if (query.isNotEmpty) {
      List<BezerroCorte> dummyListData = [];
