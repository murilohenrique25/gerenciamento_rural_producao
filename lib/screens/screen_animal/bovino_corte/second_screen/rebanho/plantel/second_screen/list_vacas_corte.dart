...
import 'package:gerenciamento_rural/screens/screen_animal/bovino_corte/second_screen/rebanho/plantel/second_screen/registers/cadastro_vaca_corte.dart';
...
class ListaVacasCorte extends StatefulWidget {
  @override
  _ListaVacasCorteState createState() => _ListaVacasCorteState();
}

class _ListaVacasCorteState extends State<ListaVacasCorte> {
  TextEditingController editingController = TextEditingController();
  VacaCorteDB helper = VacaCorteDB();
  List<VacaCorte> totalVacas = [];
  List<VacaCorte> items = [];
  List<VacaCorte> vacas = [];
  List<VacaCorte> tvacas = [];
  @override
  void initState() {
    super.initState();
    _getAllVacas();
                  ...
                  children: <Widget>[
                    ...
                    Padding(
                      ...
                      child: ElevatedButton(
                        ...
                        onPressed: () {
                          try {
                            helper.delete(totalVacas[index].id);
                          } catch (e) {
  ...
  void _showTotalVacasPage({VacaCorte totalVacas}) async {
    final recTotalVacas = await Navigator.push(
        context,
        MaterialPageRoute(
          builder: (context) => CadastroVacaCorte(
            vaca: totalVacas,
          ),
        ));
  ...
  void _getAllVacas() {
    ...
    helper.getAllItems().then((list) {
      setState(() {
        ...
        totalVacas.forEach((element) {
          print(element.partoPrevisto);
        });
      });
    });
  }
  ...
  void filterSearchResults(String query) {
    List<VacaCorte> dummySearchList = [];
    dummySearchList.addAll(items);
    if (query.isNotEmpty) {
      List<VacaCorte> dummyListData = [];
